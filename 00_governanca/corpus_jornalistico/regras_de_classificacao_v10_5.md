[regras_de_classificacao_v10_5.md](https://github.com/user-attachments/files/28178824/regras_de_classificacao_v10_5.md)
[Uploading reg---
...
**Repositório:** Atlas-Social-de-Hortolandia-core / metodologia/classificacao_analitica/
...
## Changelog v10.4 → v10.5

| Item | Mudança |
|---|---|
| Seção 11 | R28 adicionada — critério de criação de `id_caso_pressao` |
| Índice de Regras | R28 registrada |
| Changelog | Atualizado |
| Motivação | Decisão de 24/05/2026: formalizar critério que impede inflação artificial de ciclos e protege o poder analítico do `id_caso_pressao` |
...
Pertence a ciclo temático? → id_caso_pressao (opcional — R28)
    ex: CH_VIOLENCIA_GENERO_2025
    ⚠ Criar novo ciclo apenas quando atender R28
    ↓
...
### R28 — Critério de criação de `id_caso_pressao` *(NOVO v10.5)*

> Um novo `id_caso_pressao` só deve ser criado quando o conjunto de evidências disponíveis apresentar pelo menos **três dos seguintes critérios**:
>
> | # | Critério | Descrição |
> |---|---|---|
> | 1 | **Recorrência** | O fenômeno aparece em múltiplos eventos no corpus |
> | 2 | **Territorialização** | Há concentração espacial identificável |
> | 3 | **Escalada** | Existe progressão mensurável de intensidade |
> | 4 | **Persistência** | O fenômeno se mantém ao longo do tempo |
> | 5 | **Pressão institucional** | Há sobrecarga observável sobre SUAS, Saúde ou Segurança |
> | 6 | **Encadeamento causal** | Eventos distintos estão conectados por lógica causal demonstrável |
>
> **Evento isolado de GOVERNANÇA responsiva não inaugura ciclo**, mesmo que responda a pressão estrutural identificada.
>
> **Pergunta de corte (R28):**
> *"Existe uma sequência de eventos conectados que configura um fenômeno territorial em curso, ou este é um episódio único de resposta a condição estrutural permanente?"*
> - Sequência configurada com ≥ 3 critérios → criar `id_caso_pressao`
> - Episódio único ou condição estrutural sem dinâmica própria → `id_caso_pressao = vazio`
...
| `id_caso_pressao` | Agrupamento temático do ciclo — ver R28 | 3 | não |
...
| `id_caso_pressao` | `CH_VIOLENCIA_GENERO_2025` | R28: recorrência + escalada + pressão institucional |
...
## 26. Exemplo de Aplicação — R28 em contraste (24/05/2026)

| Campo | 3º Feirão de Empregos | CH_CRIMINALIDADE_2025 |
|---|---|---|
| Cria `id_caso_pressao`? | **não** | **sim** |
| Motivo | Evento isolado de GOVERNANÇA responsiva; desemprego é condição estrutural, não ciclo com dinâmica própria configurada | Recorrência + territorialização (Veccon Buriti) + pressão institucional (Dise) |
| `id_caso_pressao` | vazio | `CH_CRIMINALIDADE_2025` |

---

## 27. Distinção conceitual
...
| R28 | Critério de criação de `id_caso_pressao` | 11 |
...
| v10.5 | "24/05/2026" | R28 — critério de criação de id_caso_pressao; exemplo de aplicação R28 adicionado na Seção 26 |

---

*Arquivo de governança — Atlas Social de Hortolândia*  
*metodologia/classificacao_analitica/*
ras_de_classificacao_v10_5.md…]()
